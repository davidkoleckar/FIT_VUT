...
void ParseCommandline(int argc, char *argv[], TParameters & parameters)
{
  ...
  if (!(n_flag && t_flag && i_flag  && m_flag) || 
      !(parameters.mode >= 0 && parameters.mode <= 3))
  {
    PrintUsageAndExit();
  }
...
void PrintUsageAndExit()
{
  ...
  fprintf(stderr,"  -m [0-3]    mode 0 - run sequential version"); 
  fprintf(stderr,"              mode 1 - run parallel version (non-overlapped output)\n");
  fprintf(stderr,"              mode 2 - run parallel version using tasks(non-overlapped output)\n");
  fprintf(stderr,"              mode 3 - run parallel version (overlapped output)\n"); 
